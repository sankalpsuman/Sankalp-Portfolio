...
import { Save, Loader2, Search, X, Plus, FileUp, Link as LinkIcon, Globe } from 'lucide-react';
...
export default function SEOEditor() {
  ...
  const getTransVal = (field: 'title' | 'description'): string => {
  ...
  const setTransVal = (field: 'title' | 'description', val: string) => {
    if (activeEditorLang === 'en') {
      setValue(field, val);
    } else {
  ...
  const handleAutoTranslate = async () => {
    ...
    try {
      ...
      if (docWithTranslations.translations) {
        ...
        alert('Meta tags translated successfully! Review the different language tabs and Save.');
      }
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="max-w-2xl space-y-8 pb-20 animate-in fade-in duration-300">
      <DeleteConfirmModal 
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-[#050816] border border-white/5 rounded-2xl">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-gray-300">SEO Localization (Active: {activeEditorLang.toUpperCase()})</span>
        </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-[#050816] border border-white/5 rounded-2xl p-6 lg:p-8 space-y-6">
        ...
        <div className="space-y-6">
           <div className="space-y-2">
              <label className="text-xs text-gray-500 uppercase tracking-widest font-mono">Meta Title {activeEditorLang !== 'en' && `(${activeEditorLang.toUpperCase()})`}</label>
              <input 
                value={getTransVal('title')}
                onChange={(e) => setTransVal('title', e.target.value)}
                className={cn(
                  "w-full bg-white/5 border rounded-lg px-4 py-2 outline-none",
                  activeEditorLang === 'en' ? "border-white/10 focus:border-blue-500" : "border-purple-500/30 text-purple-300 focus:border-purple-500"
                )}
                placeholder="Sankalp Suman | AI QA Lead" 
              />
           </div>

           <div className="space-y-2">
              <label className="text-xs text-gray-500 uppercase tracking-widest font-mono font-bold">Meta Description {activeEditorLang !== 'en' && `(${activeEditorLang.toUpperCase()})`}</label>
              <textarea 
                value={getTransVal('description')}
                onChange={(e) => setTransVal('description', e.target.value)}
                rows={3} 
                className={cn(
                  "w-full bg-white/5 border rounded-lg px-4 py-2 outline-none resize-none",
                  activeEditorLang === 'en' ? "border-white/10 focus:border-blue-500" : "border-purple-500/30 text-purple-300 focus:border-purple-500"
                )}
