    ...
    <div className="bg-[#050816] min-h-screen text-white flex flex-col">
       ...
       <header className="pt-32 pb-16 px-4 border-b border-white/5 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto flex flex-col items-center text-center space-y-6">
             <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-blue-500 font-mono text-sm uppercase tracking-widest">Knowledge Base</motion.div>
             <h1 className="text-5xl md:text-7xl font-bold tracking-tight">The <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-emerald-500">QA Engine</span></h1>
             <p className="text-gray-400 max-w-2xl text-lg">Exploring the intersection of Quality Engineering, AI Automation, and Agile Excellence.</p>
             
             <div className="w-full max-w-xl relative mt-8">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 w-5 h-5" />
                <input 
                  type="text" 
                  placeholder="Search articles, categories, or tech stack..."
                  value={search}
       ...
       <main className="flex-1 max-w-7xl mx-auto px-4 py-20 w-full">
          {loading ? (
                     ...
                     <Link to={`/blog/${blog.slug}`} className="cursor-pointer space-y-6 block">
                        <div className="relative aspect-[16/10] rounded-[2.5rem] overflow-hidden bg-white/5 border border-white/10 shadow-2xl">
                           ...
                           <div className="absolute bottom-6 left-6 right-6 flex justify-between items-center opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300">
                             <span className="text-xs font-bold text-white flex items-center gap-2">Read Article <ArrowRight className="w-4 h-4" /></span>
                           </div>
                        </div>
            ...
            <div className="text-center py-20 space-y-4 opacity-30">
               ...
               <p className="font-mono text-sm uppercase tracking-widest">No articles matching your search</p>
            </div>
          )}
       </main>
